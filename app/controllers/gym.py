...
def get_gym_details(**kwargs) -> dict|None:
    ...
    if maintenance_status:
        gym_info["maintenance_status"] = queryset[0].divisions.maintenance_status if queryset else None

    if fee_structure:
        gym_info["fee_structure"] = queryset[0].divisions.fee_details if queryset else None

    if application_process:
        gym_info["application_process"] = f"Either you can apply through our HRMS website or {queryset[0].divisions.phone}" if queryset else None
